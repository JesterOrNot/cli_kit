...
function getSearchElement() { return document.getElementById("search") }
(function() {
"use strict";
var itemTypes = [
  "mod",
...
onEachLazy(document.getElementsByClassName("js-only"),
           function(e) { removeClass(e, "js-only") });
...
function highlightSourceLines(ev) {
  ...
  if (match) {
    ...
    onEachLazy(document.getElementsByClassName("line-numbers"), function(e) {
      onEachLazy(e.getElementsByTagName("span"),
                 function(i_e) { removeClass(i_e, "line-highlighted") })
    });
    for (i = from; i <= to; ++i) {
  ...
  } else if (ev !== null && search && !hasClass(search, "hidden") &&
             ev.newURL) {
...
function getHelpElement() { return document.getElementById("help") }
...
document.onclick = function(ev) {
  if (hasClass(ev.target, "collapse-toggle")) {
    ...
    collapseDocs(ev.target.parentNode, "toggle")
             ...
             hasClass(ev.target.parentNode, "line-numbers")) {
    var prev_id = 0;
...
if (x.length > 0) {
  x[0].onchange = function() {
    var i, match, url = document.location.href, stripped = "",
    ...
    url += "/" + document.getElementsByClassName("version-selector")[0].value +
           stripped;
...
function initSearch(rawSearchIndex) {
  ...
  if (savedCrate !== null) {
    onEachLazy(
        document.getElementById("crate-search").getElementsByTagName("option"),
        function(e) {
  ...
  function execQuery(query, searchWords, filterCrates) {
    ...
    var valLower = query.query.toLowerCase(), val = valLower,
        typeFilter = itemTypeFromName(query.type), results = {},
    ...
    function sortResults(results, isType) {
      ...
      for (i = 0; i < length; ++i) {
        ...
        var name = result.item.name.toLowerCase(),
            path = result.item.path.toLowerCase(), parent = result.item.parent;
        if (isType !== true &&
        ...
        if (literalSearch === true) {
          if (val.generics && val.generics.length !== 0) {
            if (obj.length > GENERICS_DATA &&
              ...
              var elems = obj[GENERICS_DATA].slice(0);
              var allFound = true;
              for (var y = 0; allFound === true && y < val.generics.length;
                   ++y) {
                allFound = false;
    ...
    if ((val.charAt(0) === "\"" || val.charAt(0) === "'") &&
      ...
      query.inputs = inputs.map(function(input) { return input.name });
  ...
  function validateResult(name, path, keys, parent) {
    for (var i = 0; i < keys.length; ++i) {
      if (!(name.indexOf(keys[i]) > -1 || path.indexOf(keys[i]) > -1 ||
            (parent !== undefined &&
  ...
  function getQuery(raw) {
    ...
    query = raw;
    matches =
        query.match(/^(fn|mod|struct|enum|trait|type|const|macro)\s*:\s*/i);
    if (matches) {
  ...
  function initSearchNav() {
    ...
    var mouseover_func = function(e) {
      ...
      hoverTimeout = setTimeout(function() {
        onEachLazy(
            document.getElementsByClassName("search-results"), function(e) {
              onEachLazy(e.getElementsByClassName("result"),
                         function(i_e) { removeClass(i_e, "highlighted") })
            });
    ...
    search_input.onkeydown = function(e) {
      ...
      onEachLazy(document.getElementById("results").childNodes, function(e) {
        onEachLazy(e.getElementsByClassName("highlighted"),
                   function(e) { actives[current].push(e) });
        current += 1
      });
      if (e.which === 38) {
  ...
  function buildHrefAndPath(item) {
    ...
    if (type === "mod") {
      ...
      displayPath = "";
      href = rootPath + item.path.replace(/::/g, "/") + "/" + type + "." +
             name + ".html"
      ...
      href = rootPath + item.path.replace(/::/g, "/") + "/" + parentType + "." +
    ...
    if (array.length > 0) {
      ...
      array.forEach(function(item) {
        ...
        output += "<tr class=\"" + type + " result\"><td>" +
                  "<a href=\"" + item.href + "\">" +
                  (item.is_alias === true
                       ? ("<span class=\"alias\"><b>" + item.alias +
                          " </b></span><span " +
                          "class=\"grey\"><i>&nbsp;- see&nbsp;</i></span>")
                       : "") +
                  item.displayPath + "<span class=\"" + type + "\">" + name +
                  "</span></a></td><td>" +
                  "<a href=\"" + item.href + "\">" +
      ...
      output =
          "<div class=\"search-failed\"" + extraStyle + ">No results :(<br/>" +
  ...
  function makeTabHeader(tabNb, text, nbElems) {
    if (currentTab === tabNb) {
      return "<div class=\"selected\">" + text + " <div class=\"count\">(" +
             nbElems + ")</div></div>"
    }
    return "<div>" + text + " <div class=\"count\">(" + nbElems +
           ")</div></div>"
  }
  function showResults(results) {
    ...
    var output =
        "<h1>Results for " + escape(query.query) +
        (query.type ? " (type: " + escape(query.type) + ")" : "") + "</h1>" +
        "<div id=\"titles\">" + makeTabHeader(0, "In Names", ret_others[1]) +
        makeTabHeader(1, "In Parameters", ret_in_args[1]) +
        makeTabHeader(2, "In Return Types", ret_returned[1]) +
        "</div><div id=\"results\">" + ret_others[0] + ret_in_args[0] +
        ret_returned[0] + "</div>";
    ...
    onEachLazy(search.getElementsByClassName("desc"),
               function(e) { e.style.width = width + "px" });
    initSearchNav();
  ...
  function execSearch(query, searchWords, filterCrates) {
    ...
    if (queries.length > 1) {
      ...
      return {
        "in_args": results.in_args[0], "returned": results.returned[0],
...
window.register_implementors = function(imp) {
  ...
  var synthetic_implementors =
      document.getElementById("synthetic-implementors-list");
  ...
  for (var i = 0; i < llength; ++i) {
    ...
    struct_loop: for (var j = 0; j < slength; ++j) {
      ...
      display.innerHTML =
          ...
          "<tbody><tr><td><code>" + code.outerHTML +
          "</code></td><td></td></tr>" +
          "</tbody></table></span>";
...
function toggleAllDocs(pageId, fromAutoCollapse) {
  ...
  if (hasClass(innerToggle, "will-expand")) {
    ...
    onEveryMatchingChild(
        innerToggle, "inner",
        function(e) { e.innerHTML = labelForToggleButton(false) });
    innerToggle.title = "collapse all docs";
    if (fromAutoCollapse !== true) {
      onEachLazy(document.getElementsByClassName("collapse-toggle"),
                 function(e) { collapseDocs(e, "show") })
    }
  } else {
...
function autoCollapse(pageId, collapse) {
  if (collapse) {
    ...
    if (impl_list !== null) {
      onEachLazy(impl_list.getElementsByClassName("collapse-toggle"),
                 function(e) { collapser(e, collapse) })
    }
    var blanket_list = document.getElementById("blanket-implementations-list");
    if (blanket_list !== null) {
      onEachLazy(blanket_list.getElementsByClassName("collapse-toggle"),
                 function(e) { collapser(e, collapse) })
    }
  }
}
...
function createSimpleToggle(sectionIsCollapsed) {
  ...
  toggle.innerHTML = "[<span class=\"inner\">" +
                     labelForToggleButton(sectionIsCollapsed) + "</span>]";
  return toggle
}
...
var showItemDeclarations =
    getCurrentValue("rustdoc-item-declarations") === "false";
function buildToggleWrapper(e) {
  if (hasClass(e, "autohide")) {
    ...
    if (wrap && hasClass(wrap, "toggle-wrapper")) {
      ...
      onEachLazy(inner_toggle.getElementsByClassName("inner"),
                 function(e) { e.innerHTML = labelForToggleButton(true) });
      onEachLazy(inner_toggle.getElementsByClassName("toggle-label"),
                 function(e) {
                   e.style.display = "inline-block";
                   if (extra === true) {
  ...
  if (e.parentNode.id === "main") {
    ...
    e.parentNode.insertBefore(createToggle(otherMessage, fontSize, extraClass,
...
if (getCurrentValue("rustdoc-item-attributes") !== "false") {
  itemAttributesFunc = function(
      x) { collapseDocs(x.previousSibling.childNodes[0], "toggle") }
}
...
onEachLazy(document.getElementsByClassName("rust-example-rendered"), function(
                                                                         e) {
...
onEachLazy(document.getElementsByClassName("important-traits"), function(e) {
  e.onclick = function() { showModal(e.lastElementChild.innerHTML) }
});
...
if (search_input) {
  search_input.onfocus = function() { putBackSearch(this) }
}
var params = getQueryStringParams();
...
window.onresize = function() { hideSidebar() };
autoCollapse(getPageId(), getCurrentValue("rustdoc-collapse") === "true");
if (window.location.hash && window.location.hash.length > 0) {
  expandSection(window.location.hash.replace(/^#/, ""))
}
if (main) {
  onEachLazy(main.getElementsByClassName("loading-content"),
             function(e) { e.remove() });
...
function focusSearchBar() { getSearchInput().focus() }
function defocusSearchBar() { getSearchInput().blur() }